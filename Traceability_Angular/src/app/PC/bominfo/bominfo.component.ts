...
export class BOMInfoComponent {
  ...
  BOMDetailDownload() {
    this.BOMService.IsShowLoading = true;
    this.BOMDetailService.ExportByParentIDToExcelAsync().subscribe(
      res => {
        let url = (res as BaseResult).Message;
        window.open(url, "_blank");
      },
  ...
  BOMDetailSaveList() {
    this.BOMService.IsShowLoading = true;
    this.BOMDetailService.BaseParameter.List = this.BOMDetailService.List;
    this.BOMDetailService.SaveListAsync().subscribe(
      res => {
        this.BOMDetailSearch();
        this.NotificationService.warn(environment.SaveSuccess);
      },
