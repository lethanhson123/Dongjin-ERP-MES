...
function Buttonexport_Click() {
    ...
    try {
        ...
        TableHTMLToExcel('TempExport', 'IQC_NG_History.xls', 'IQC_NG_History');
        $exportTable.remove();
...
async function Buttonfind_Click() {
    try {
        ...
        const data = await callApi("/F01/Buttonfind_Click", {
            FromDate: $("#tab2_fromDate").val(),
            ToDate: $("#tab2_toDate").val(),
            SearchText: $("#tab2_Search").val().trim()
        });

        if (data.ErrorNumber === 0) {
            BaseResult = data;
            RenderTable2(data.HistoryDataList || []);
            ShowMessage(data.Message || `Found ${data.HistoryDataList?.length || 0} records`, "ok");
        } else {
...
function RenderTable2(dataList) {
    ...
    // ✅ FIX: Chỉ tính totalInvoice 1 lần cho mỗi MaterialCode
    let totalInvoice = 0;
    ...
    dataList.forEach((item, index) => {
        ...
        // ... phần render table giữ nguyên (không đổi) ...
        ...
        const percentage = item.Percentage ? item.Percentage.toFixed(2) + '%' : '0%';
...
function EditHistory(id) {
    ...
    $("#edit_Unit").val(CurrentHistoryData.Unit || 'PCS');
...
async function SaveHistory() {
    ...
    try {
        ...
        const data = await callApi("/F01/ButtonUpdateHistory_Click", {
...
async function Buttondelete_Click() {
    ...
    try {
        ...
        const data = await callApi("/F01/Buttondelete_Click", {
            Ids: checkedIds
        });

        if (data.ErrorNumber === 0) {
            ShowMessage(data.Message || "Delete successfully!", "ok");
        ...
        ShowMessage("Error: " + err.message, "error");
        console.error("Buttondelete_Click:", err);
    } finally {
        showLoading(false);
    }
}
...
async function SaveNG() {
    ...
    const total = parseFloat($('#txt_Total').text().trim()) || 0;
    ...
    try {
        const data = await callApi("/F01/Buttonsave_Click", {
            ID: (id && id !== '-') ? parseInt(id) : 0,
            LotCode: lotCode,
            PartNo: (productCode !== '-') ? productCode : '',
            PartName: (productName !== '-') ? productName : '',
            Invoice: (invoice !== '-') ? invoice : '',
            GroupCode: (supplier !== '-') ? supplier : '',
            Total: total,
        ...
        if (data?.ErrorNumber === 0) {
            ShowMessage(data.Message || "Save successfully!", "ok");
            clearForm();
        } else {
...
function clearForm() {
    $('#txt_ID, #txt_LotCode, #txt_ProductCode, #txt_ProductName, #txt_Invoice, #txt_Supplier, #txt_Total, #txt_Picture').text('-');
    $('#error_Qty, #error_Infor, #tab1_QRCodeScan').val('');
...
function updateUIWithLotcodeInfo(item) {
    ...
    $("#txt_Total").text(item.Quantity || item.TotalQty || item.Qty || '-');
    $("#txt_TotalInvoice").text(item.QuantityInvoice || '-');
    $("#txt_Unit").text(item.CategoryUnitName || 'PCS');
}

function clearLotcodeFields() {
    $("#txt_ID, #txt_ProductCode, #txt_LotCode, #txt_ProductName, #txt_Invoice, #txt_Supplier, #txt_Total, #txt_TotalInvoice").text('-');
    $("#tab1_QRCodeScan").focus();
}
...
async function UploadPicture() {
    ...
    formData.append("file", selectedFile, `IQCNG_${Date.now()}.jpg`);

    try {
        const response = await fetch("/F01/UploadPhoto", {
            method: "POST",
            body: formData
        });
