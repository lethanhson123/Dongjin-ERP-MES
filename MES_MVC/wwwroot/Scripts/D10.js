...
async function PrintLabel() {
    ...
    if (partCode && SNP && Line && Family && TotalQty && PackingLot) {
        ...
        const url = `/html/ZDA_Lable.html?PartNo=${valuesSend.PartNo}&SerialNo=${PackingLot}&MfgDate=${valuesSend.MfgDate}&Description=${Family}&Line=${Line}&Quantity=${TotalQty}`;
...
async function FirstScan(lotCode) {
    ...
    const postAddScan = async (part_Code, ecnNo) => {
    ...
    if (lotCode.includes("DJG#")) {
        ...
        if (!part_Code || !ecnNo) return scanFailed();

        await postAddScan(part_Code, ecnNo);
        return;
    }

    const parts = lotCode.trim().split(/\s+/);
    if (parts.length === 2) {
        let chuoi = parts[0].trim();
        const part_Code = chuoi.substring(5);
        const ecnNo = getECN(lotCode);
        if (!part_Code || !ecnNo) return scanFailed();
    ...
    if (parts.length === 3 && parts[1].length >= 11) {
        const part_Code = parts[1];
        const ecnNo = getECN(lotCode);
        if (!part_Code || !ecnNo) return scanFailed();
...
async function NextScan(lotCode) {
    try {
        const PartCodeInput = $("#tab1_PartCode").val()?.trim() || "";
        const ECNInput = $("#tab1_ECN").val()?.trim() || "";
        const SNP = $("#tab1_SNP").val()?.trim() || "";
        ...
        if (lotCode.includes("DJG#")) {
            part_Code = getPartNo(lotCode);
            if (!part_Code) throw new Error(localizedText.WrongCode);

            ecnNo = getECN(lotCode);
            if (!ecnNo) throw new Error(localizedText.WrongCode);
        } else {
            const parts = lotCode.trim().split(/\s+/);
            if (parts.length === 2) {
                let chuoi = parts[0].trim();
                part_Code = chuoi.substring(5);
            } else if (parts.length === 3) {
                part_Code = parts[1].trim();             
        ...
        // So sánh PartCode và ECN
        if (PartCodeInput.replace(/\s+/g, "") !== part_Code.replace(/\s+/g, "") || ECNInput !== ecnNo) {
            ShowMessage(localizedText.WrongProduct, 'error');
            return;
        }
    ...
    try {
        ...
        if (inputString.includes("DJG#")) {
            if (inputString.includes("@@$$")) {
                ...
                if (inputString.substring(0, 3).toUpperCase() === "A 1") {
                    return inputString.substring(0, 15).trim();
                } else {
                    const parts = lotCode.trim().split(/\s+/);
                    if (parts.length === 2) {
...
function getECN(lotCode) {
    if (lotCode.includes("DJG#")) {
        if (lotCode.includes("@@$$")) {
            ...
            let trimmedText = lotCode.trim().toUpperCase().replace(/-/g, "");
...
async function ExportToExcelFull() {
    ...
    try {
        ...
        if (data.ListtspartTranfer && data.ListtspartTranfer.length > 0) {
            ...
            ExportJSONToExcel(exportData, "Scan history", "D10_ScanHistory.xlsx");
