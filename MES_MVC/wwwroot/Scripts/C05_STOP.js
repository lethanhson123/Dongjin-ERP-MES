...
function SW_TIME() {
    let USER_MC = localStorage.getItem("C05_MCbox");
    let USER_ORIDX = localStorage.getItem("C05_START_V2_Label8");
    let Label5 = $("#Label5").val();;
    $("#BackGround").css("display", "block");
    let BaseParameter = new Object();
    BaseParameter = {
...
function OPER_TIME() {
    ...
    fetch(url, {
        ...
        response.json().then((data) => {
            ...
            localStorage.setItem("C05_START_V2_BI_STOPTIME", BaseResultOPER_TIME.Error);
            $("#BackGround").css("display", "none");
        }).catch((err) => {
            $("#BackGround").css("display", "none");
        })
...
function C05_STOP_FormClosed_Tick() {
    $("#BackGround").css("display", "block");
    setInterval(function () {
        localStorage.setItem("C05_STOP_Close", 1);
        $("#BackGround").css("display", "none");
        window.close();
    }, 5000);
}
...
function Button2_Click() {
    ...
    let formUpload = new FormData();
    formUpload.append('BaseParameter', JSON.stringify(BaseParameter));
    let url = "/C05_STOP/Button2_Click";
