...
$(document).ready(function () {
    ...
    localStorage.setItem("C11_2_Close", 0);

    BaseResult.R_DGV = JSON.parse(localStorage.getItem("C11_R_DGV"));
    BaseResult.DGV_RH = JSON.parse(localStorage.getItem("C11_2_DGV_RH"));
    DGV_RHRender();

    C_EXIT = localStorage.getItem("C11_2_C_EXIT");
    $("#C11_D04").val(localStorage.getItem("C11_2_C11_D04"));
    $("#ST_DWIRE1").val(localStorage.getItem("C11_2_ST_DWIRE1"));
    $("#WIRE_Length").val(localStorage.getItem("C11_2_WIRE_Length"));
    $("#ST_DSTR1").val(localStorage.getItem("C11_2_ST_DSTR2"));
    $("#ST_DCC1").val(localStorage.getItem("C11_2_ST_DCC1"));
    $("#ST_DIC1").val(localStorage.getItem("C11_2_ST_DIC1"));
    $("#C11_COUNT1").val(localStorage.getItem("C11_2_C11_COUNT1"));
    $("#C11_COUNT3").val(localStorage.getItem("C11_2_C11_COUNT3"));
    $("#C11_D01").val(localStorage.getItem("C11_2_C11_D01"));
    $("#ORDER").val(localStorage.getItem("C11_2_ORDER"));
        ...
        response.json().then((data) => {            
            ...
            if (BaseResult.Search.length <= 0) {
                ...
                try {                    
                    ...
                    $("#Label1").val(Number($("#C11_COUNT2").val()) * BaseResult.DGV_RH.length);
                    ...
                    $("#C11_COUNT2").val("");
                    $("#VLT1").val("");
                    localStorage.setItem("C11_ER_R_LBT2S", BaseResult.R_DGV[0].TERM1);
                    localStorage.setItem("C11_ER_R_LBA2S", BaseResult.R_DGV[0].TERM1);
                    localStorage.setItem("C11_ER_R_Label1", localStorage.getItem("C11_ST_BCIDX"));

                    BaseResult.DGV_RH = [];
                    DGV_RHRender();

                    localStorage.setItem("C11_ER_R_Close", 0);
                    localStorage.setItem("C11_ER_R_LBT2S", BaseResult.R_DGV[0].TERM1);
                    localStorage.setItem("C11_ER_R_LBA2S", BaseResult.R_DGV[0].TERM1);
                    localStorage.setItem("C11_ER_R_Label1", localStorage.getItem("C11_ST_BCIDX"));
...
function C11_2_FormClosed() {
    localStorage.setItem("C11_2_Close", 1);
    window.close();
}
...
function SPC1_Click() {
    ...
    localStorage.setItem("C11_SPC_R_Label10", "First");
    localStorage.setItem("C11_SPC_R_Label6", $("#C11_D03").val());
    //let url = "/C11_SPC_R";
    //OpenWindowByURL(url, 800, 460);
    //C11_SPC_RTimerStartInterval(0);

    $("#C11_SPC_R_Label10").val("First");
    $("#C11_SPC_R_Label6").val($("#C11_D03").val());
    C11_SPC_R_Load();
    $("#C11_SPC_R").modal("open");
}
function C11_SPC_RTimerStartInterval(Flag) {
    let C11_SPC_RTimer = setInterval(function () {
        let C11_SPC_R_Close = localStorage.getItem("C11_SPC_R_Close");
        if (C11_SPC_R_Close == "1") {
            clearInterval(C11_SPC_RTimer);
            C11_SPC_R_Close = 0;
            localStorage.setItem("C11_SPC_R_Close", C11_SPC_R_Close);
            if (Flag == 0) {
                SPC_LOAD();
            }
            if (Flag == 1) {
...
function SPC2_Click() {
    ...
    if (C11_COUNT1 > 501) {
        if (C11_COUNT1 / 2 <= C11_COUNT2) {
            localStorage.setItem("C11_SPC_R_Close", 0);
            localStorage.setItem("C11_SPC_R_Label10", "Middle");
            localStorage.setItem("C11_SPC_R_Label6", $("#C11_D03").val());
            //let url = "/C11_SPC_R";
            //OpenWindowByURL(url, 800, 460);
            //C11_SPC_RTimerStartInterval(0);
...
function SPC3_Click() {
    ...
    if (BB <= 0) {
        localStorage.setItem("C11_SPC_R_Close", 0);
        localStorage.setItem("C11_SPC_R_Label10", "End");
        localStorage.setItem("C11_SPC_R_Label6", $("#C11_D03").val());
        //let url = "/C11_SPC_R";
        //OpenWindowByURL(url, 800, 460);
        //C11_SPC_RTimerStartInterval(0);
...
function Buttonprint_ClickSub() {
    ...
    if (IsCheck == true) {
        let C11_COUNT3 = Number($("#C11_COUNT3").val());
        let TM_QTY = C11_COUNT3 - BaseResult.DGV_RH.length;
        $("#Label1").val(TM_QTY);
...
function BT_APP1_Click() {
    ...
    if (IsCheck == true) {
        ...
        localStorage.setItem("C11_APPLICATION_Label3", "APPLICATION #R");
...
function DGV_RHRender() {
    let HTML = "";
    if (BaseResult) {
        if (BaseResult.DGV_RH) {
            if (BaseResult.DGV_RH.length > 0) {
                DGV_RH_SelectionChanged(0);
                for (let i = 0; i < BaseResult.DGV_RH.length; i++) {
                    HTML = HTML + "<tr onclick='DGV_RH_SelectionChanged(" + i + ")'>";
                    HTML = HTML + "<td>" + BaseResult.DGV_RH[i].NO + "</td>";
                    HTML = HTML + "<td>" + BaseResult.DGV_RH[i].TERM1 + "</td>";
                    HTML = HTML + "</tr>";
                }
            }
        }
    }
    document.getElementById("DGV_RH").innerHTML = HTML;
}
function DGV_RH_SelectionChanged(i) {
    DGV_RHRowIndex = i;
}
let DGV_RHTable = document.getElementById("DGV_RHTable");
DGV_RHTable.addEventListener('click', function (event) {
    if (event.target.tagName === 'TH') {
        let text = event.target.innerText;
        let key = "NO";
        IsTableSort = !IsTableSort;
        ListSort(BaseResult.DGV_RH, key, text, IsTableSort);
        DGV_RHRender();
    }
});
...
function C11_SPC_R_Load() {
    ...
    $("#C11_SPC_R_ST11").val($("#ST_DCC1").val());
    $("#C11_SPC_R_ST12").val($("#ST_DIC1").val());
    $("#C11_SPC_R_Label8").val(localStorage.getItem("C11_VLW"));
    $("#C11_SPC_R_Label18").val($("#WIRE_Length").val());
    $("#C11_SPC_R_Label19").val($("#ST_DWIRE1").val());
    ...
    $("#C11_SPC_R_Label4").val($("#VLT1").val());
    $("#C11_SPC_R_Label5").val($("#VLT1").val());
...
function C02_SPC_Load() {
    ...
    fetch(url, {
        ...
        response.json().then((data) => {
            ...
            $("#C11_SPC_R_Label17").val(BaseResult.Search[0].STRENGTH);
            $("#BackGround").css("display", "none");
        }).catch((err) => {
            $("#BackGround").css("display", "none");
        })
