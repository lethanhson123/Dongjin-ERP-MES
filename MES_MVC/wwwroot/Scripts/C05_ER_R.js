...
$(document).ready(function () {
    ...
    $("#LBT2S").val(localStorage.getItem("C05_ER_R_LBT2S"));
    $("#LBA2S").val(localStorage.getItem("C05_ER_R_LBA2S"));
    $("#Label1").val(localStorage.getItem("C05_ER_R_Label1"));
...
function C11_ERROR_FormClosed() {
    ...
    fetch(url, {
        ...
        response.json().then((data) => {
            ...
            localStorage.setItem("C05_ER_R_Close", 1);
            window.close();
        }).catch((err) => {
            $("#BackGround").css("display", "none");
        })
