...
@Component({
  selector: 'app-warehouse-output-admin',
  templateUrl: './warehouse-output-admin.component.html',
  styleUrls: ['./warehouse-output-admin.component.css']
})
export class WarehouseOutputAdminComponent {
  ...
  WarehouseOutputSearch() {
    ...
    this.WarehouseOutputService.GetByCompanyID_CategoryDepartmentID_Action_DateBegin_DateEnd_SearchStringToListAsync().subscribe(
      res => {
  ...
  CreateAutoAsync() {
    this.WarehouseOutputService.IsShowLoading = true;
    this.WarehouseOutputService.CreateAutoAsync().subscribe(
      res => {
      },
      err => {
      },
