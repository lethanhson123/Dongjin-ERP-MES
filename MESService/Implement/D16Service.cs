namespace MESService.Implement
{
    ...
    public virtual async Task<BaseResult> Load()
    {
            ...
            try
            {
                string sql = @"SELECT (IFNULL(MAX(`SEQ_NO`),0)+1) AS SEQ_NO  FROM tdpdmtin_serial WHERE `TDPD_DATE`= @CurentDate  AND Serial_no LIKE '%DH%'";
     ...
     public virtual async Task<BaseResult> Buttonadd_Click(BaseParameter BaseParameter)
     {
            ...
            try
            {
                if (!string.IsNullOrEmpty(BaseParameter.Barcode))
                {
                    ...
                    {
                        result = await AddNewIDCardAsync(barcode, partNo, User_ID, BaseParameter.CustomerCode, BaseParameter.CreateDate, BaseParameter.BoxNumber);
                    }
                }
            }
            catch (Exception ex)
    ...
    public virtual async Task<BaseResult> Buttonclose_Click(BaseParameter BaseParameter)
    {
        ...
        private async Task<BaseResult> AddNewIDCardAsync(string barcode, string partNo, string User_ID, string customerCode, DateTime? CreateDate, int? Boxnumber)
        {
            BaseResult result = new BaseResult();
        ...
        private async Task<string> CreatePackingLotAsync(BaseResult result, string customerCode, string Create_User)
        {
            ...
            string packingLot = curentDate.ToString("yyMMdd") + int.Parse(number).ToString("000") + "DH";
