...
{
    public class C02_STOPService : BaseService<torderlist, ItorderlistRepository>, IC02_STOPService
    {
        ...
        // ============================================
        // CODE CŨ: PageLoad (GIỮ NGUYÊN + THÊM LOGIC)
        // ============================================
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        ('" + STOP_MC + "', '" + C_USER + "', DATE_FORMAT(NOW(), '%Y-%m-%d'), NOW(), '" + ST_TM + "', '" + C_USER + "', '" + Label5 + "','New MES C02 STOP')";
                        ...
                        if (Label5 == "M")
                        {
                            sql = @"INSERT INTO  `tsnon_oper_andon` 
                            (`tsnon_oper_mitor_MCNM`, `tsnon_oper_mitor_NOIC`, `tsnon_oper_mitor_RUNYN`, `CREATE_DTM`, `CREATE_USER`) VALUES 
                            ('" + STOP_MC + "', 'MC STOP', 'Y', NOW(), '" + C_USER + "') ON DUPLICATE KEY UPDATE `tsnon_oper_mitor_NOIC` = VALUES(`tsnon_oper_mitor_NOIC`), `tsnon_oper_mitor_RUNYN` = VALUES(`tsnon_oper_mitor_RUNYN`), `UPDATE_DTM` = VALUES(`CREATE_DTM`),  `UPDATE_USER` = VALUES(`CREATE_USER`)     ";
                            sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                            ...
                            // ============================================
                            // CODE MỚI: Insert MaintenanceHistory (BẮT ĐẦU)
                            // ============================================
                            try
                            {
                            ...
                            {
                                // Log error nhưng không interrupt main flow
        ...
        // ============================================
        // CODE CŨ: Tất cả method khác giữ nguyên
        // ============================================
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sql = @"INSERT INTO  `TORDERLIST_WTIME`  (`USER_ID`, `USER_MC`, `ORDER_IDX`, `S_TIME`, `CREATE_DTM`, `CREATE_USER`, `MENU_TEXT`) VALUES 
                        ('" + USER_ID + "', '" + USER_MC + "', '" + USER_ORIDX + "', NOW(), '" + ST_TM + "', '" + USER_ID + "', '" + Label5 + "')";
                        string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
        ...
        public virtual async Task<BaseResult> C02_STOP_FormClosed(BaseParameter BaseParameter)
        {
            BaseResult result = new BaseResult();
            try
            {
        ...
        public virtual async Task<BaseResult> OPER_TIME(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    DataSet ds = await MySQLHelper.FillDataSetBySQLAsync(GlobalHelper.MariaDBConectionString, sql);
                    var OPER_TIME = new List<SuperResultTranfer>();
                    for (int i = 0; i < ds.Tables.Count; i++)
                    {
                        DataTable dt = ds.Tables[i];
                        OPER_TIME.AddRange(SQLHelper.ToList<SuperResultTranfer>(dt));
                    }
                    if (OPER_TIME.Count > 0)
                    {
                        var TOT_SUM = OPER_TIME[0].SUM_TIME;
                        var H_TIME = TOT_SUM / 60 / 60;
                        TOT_SUM = TOT_SUM - (H_TIME * 60 * 60);
                        var M_TIME = TOT_SUM / 60;
                        var S_TIME = TOT_SUM - (M_TIME * 60);
                        var H_TIMEString = H_TIME.ToString();
                        if (H_TIME < 10)
                        {
                            H_TIMEString = "0" + H_TIMEString;
                        }
                        var M_TIMEString = M_TIME.ToString();
                        if (M_TIME < 10)
                        {
                            M_TIMEString = "0" + M_TIMEString;
                        }
                        var S_TIMEString = S_TIME.ToString();
                        if (S_TIME < 10)
                        {
                            S_TIMEString = "0" + S_TIMEString;
                        }
                        result.Error = H_TIMEString + ":" + M_TIMEString + ":" + S_TIMEString;
                    }
                }
            }
            catch (Exception ex)
        ...
        public virtual async Task<BaseResult> Button2_Click(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    string sql = @"INSERT INTO  `tsnon_oper_andon` (`tsnon_oper_mitor_MCNM`, `tsnon_oper_mitor_NOIC`, `tsnon_oper_mitor_RUNYN`, `CREATE_DTM`, `CREATE_USER`) VALUES
                    ('" + STOP_MC + "', 'MC STOP', 'I', NOW(), '" + C_USER + "') ON DUPLICATE KEY UPDATE `tsnon_oper_mitor_NOIC` = VALUES(`tsnon_oper_mitor_NOIC`), `tsnon_oper_mitor_RUNYN` = VALUES(`tsnon_oper_mitor_RUNYN`), `UPDATE_DTM` = VALUES(`CREATE_DTM`),  `UPDATE_USER` = VALUES(`CREATE_USER`)     ";
                    string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);

                    sql = @"ALTER TABLE     `tsnon_oper_andon`     AUTO_INCREMENT= 1";
                    sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);

                    sql = @"INSERT INTO  `tsnon_oper_andon_LIST` 
                    (`tsnon_oper_mitor_MCNM`, `tsnon_oper_mitor_NOIC`, `tsnon_oper_mitor_RUNYN`, `CREATE_DTM`, `CREATE_USER`) VALUES 
                    ('" + STOP_MC + "', 'MC STOP', 'I', NOW(), '" + C_USER + "')     ";
                    sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
        ...
        public virtual async Task<BaseResult> SaveMaintenanceHistory(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    // string reason = BaseParameter.Reason; // ← ĐÃ XÓA
                    ...
                    // Update MaintenanceHistory với chi tiết (ĐÃ XÓA REASON)
                    maintenanceHistory.CurrentStatus = currentStatus;
                    // maintenanceHistory.Reason = reason; // ← ĐÃ XÓA
