...
{
    public class C11_ER_RService : BaseService<torderlist, ItorderlistRepository>
    , IC11_ER_RService
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C11_ER_RService(ItorderlistRepository torderlistRepository
        ...
        public virtual async Task<BaseResult> C11_ERROR_Load(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    string sql = @"INSERT INTO `TSNON_OPER` (`TSNON_OPER_MCNM`, `TSNON_OPER_USERNM`, `TSNON_OPER_DATE`, `TSNON_OPER_STIME`, `CREATE_DTM`, `CREATE_USER`, `TSNON_OPER_CODE`, `REMARK`) VALUES ('" + MCbox + "', '" + USER + "', DATE_FORMAT(NOW(), '%Y-%m-%d'), NOW(), '" + ST_TM + "', '" + USER + "', 'S', 'New MES C11 ER R')";
                ...
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        if (IsCheck == true)
                        {
                            ...
                            sql = @"UPDATE torder_bom_sw  SET `T1_TOOL_IDX` = '" + TOOLMASTER_IDX + "' WHERE  `ORDER_IDX` ='" + Label1 + "' AND `LOC_LRJ` = 'R'";
                            sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        }
                    }
                }
        ...
        public virtual async Task<BaseResult> Button2_Click(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    string sql = @"UPDATE `torder_bom_sw` SET `ERROR_CHK`='Y', `UPDATE_DTM` = NOW(), `UPDATE_USER` = '" + USER_ID + "' WHERE  `ORDER_IDX`='" + Label1 + "'  AND `LOC_LRJ` = 'R'";
                    string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                }
            }
            catch (Exception ex)
