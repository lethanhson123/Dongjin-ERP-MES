...
{
    public class C02_APPLICATIONService : BaseService<torderlist, ItorderlistRepository>
    , IC02_APPLICATIONService
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C02_APPLICATIONService(ItorderlistRepository torderlistRepository
            ...
            try
            {
                if (BaseParameter != null)
                {
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        {
                            string sql = @"UPDATE `torder_bom` SET `T1_TOOL_IDX`='" + Label10 + "', `UPDATE_DTM`= NOW(), `UPDATE_USER`= '" + USER_ID + "' WHERE  `ORDER_IDX`= '" + Label8 + "'";
                            string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        }
                        if (Label3 == "APPLICATION #2")
                        {
                            string sql = @"UPDATE `torder_bom` SET `T2_TOOL_IDX`='" + Label10 + "', `UPDATE_DTM`= NOW(), `UPDATE_USER`= '" + USER_ID + "' WHERE  `ORDER_IDX`= '" + Label8 + "'";
                            string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        }
                    }
                }
