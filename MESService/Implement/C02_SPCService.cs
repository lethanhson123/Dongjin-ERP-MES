...
{
    public class C02_SPCService : BaseService<torderlist, ItorderlistRepository>
    , IC02_SPCService
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C02_SPCService(ItorderlistRepository torderlistRepository
        ...
        public virtual async Task<BaseResult> ORDER_CHG(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    string sqlResult = "";
                    string sql = @"Update TORDERLIST SET `CONDITION` = 'Working' , `UPDATE_DTM` = NOW(), `UPDATE_USER` = '" + USER_ID + "' WHERE (`ORDER_IDX` = " + OR_IDX + ")";
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        var Label8 = BaseParameter.ListSearchString[9] == "" ? "0" : BaseParameter.ListSearchString[9];
                        var Label10 = BaseParameter.ListSearchString[10] == "" ? "0" : BaseParameter.ListSearchString[10];
                        var Label11 = BaseParameter.ListSearchString[11] == "" ? "0" : BaseParameter.ListSearchString[11];
                        var MRUS = BaseParameter.ListSearchString[12] == "" ? "0" : BaseParameter.ListSearchString[12];
                        var TextBox9 = BaseParameter.ListSearchString[13] == "" ? "0" : BaseParameter.ListSearchString[13];

                        string sql = @"INSERT INTO `torderinspection` (`ORDER_IDX`, `CCH1`, `CCW1`, `CCH2`, `CCW2`, `ICH1`, `ICW1`, `ICH2`, `ICW2`, `WIRE_FORCE`, `IN_RESILT`, `COLSIP`,`CREATE_DTM`,`CREATE_USER`,`WIRE_LENGTH`) VALUES (" + Label8 + ", " + TextBox1 + ", " + TextBox2 + ", " + TextBox5 + ", " + TextBox6 + ", " + TextBox3 + ", " + TextBox4 + ", " + TextBox7 + ", " + TextBox8 + ", " + TextBox9 + ", '" + MRUS + "', '" + Label10 + "', NOW(),'" + USER_ID + "', " + TextBox10 + ") ON DUPLICATE KEY UPDATE `CCH1`= VALUES(`CCH1`), `CCW1`= VALUES(`CCW1`), `CCH2`= VALUES(`CCH2`), `CCW2`= VALUES(`CCW2`), `ICH1`= VALUES(`ICH1`), `ICW1`= VALUES(`ICW1`), `ICH2`= VALUES(`ICH2`), `ICW2`= VALUES(`ICW2`), `WIRE_FORCE`= VALUES(`WIRE_FORCE`), `IN_RESILT`= VALUES(`IN_RESILT`), `WIRE_LENGTH`= VALUES(`WIRE_LENGTH`)";
                        string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);

                        sql = @"ALTER TABLE     `torderinspection`     AUTO_INCREMENT= 1";
                        sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        result.Success = true; 
                    }
                }
