...
{
    public class C09_STOPService : BaseService<torderlist, ItorderlistRepository>
     , IC09_STOPService
    {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        // CODE CŨ - GIỮ NGUYÊN
                        string sql = @"INSERT INTO `TSNON_OPER` (`TSNON_OPER_MCNM`, `TSNON_OPER_USERNM`, `TSNON_OPER_DATE`, `TSNON_OPER_STIME`, `CREATE_DTM`, `CREATE_USER`, `TSNON_OPER_CODE`, `REMARK`) VALUES ('" + STOP_MC + "', '" + C_USER + "', DATE_FORMAT(NOW(), '%Y-%m-%d'), NOW(), '" + ST_TM + "', '" + C_USER + "', '" + Label5 + "','New MES C09 STOP')";
                        ...
                        if (Label5 == "M")
                        {
                            ...
                            sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                            // CODE MỚI: Insert MaintenanceHistory
                            try
                            {
        ...
        public virtual async Task<BaseResult> C09_STOP_FormClosed(BaseParameter BaseParameter)
        {
            BaseResult result = new BaseResult();
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        var Label5 = BaseParameter.ListSearchString.Count > 3 ? BaseParameter.ListSearchString[3] : ""; // ← THÊM
                        ...
                        string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        // CODE MỚI: Auto-close MaintenanceHistory
                        if (Label5 == "M")
                        {
                            try
                            {
