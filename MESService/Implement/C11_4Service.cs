...
{
    public class C11_4Service : BaseService<torderlist, ItorderlistRepository>
    , IC11_4Service
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C11_4Service(ItorderlistRepository torderlistRepository
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sql = @"SELECT `ORDER_IDX`, `LOC_LRJ`, `PERFORMN`, `DSCN_YN`, IFNULL(`T1_TOOL_IDX`, 0) AS `T1_TOOL_IDX`, `ERROR_CHK`,
                        ...
                        FROM torder_bom_sw `A` WHERE `ORDER_IDX` = '" + ORDER + "' AND `LOC_LRJ` = 'J2'";
                        ...
                        {
                            if (result.Search1[0].PERFORMN == result.Search1[0].TOT_COUNT)
                            {
                                sql = @"UPDATE `torder_bom_sw` SET `DSCN_YN`='Y', `UPDATE_DTM` = NOW(), `UPDATE_USER` = '" + USER_ID + "' WHERE  `ORDER_IDX`= '" + ORDER + "'  AND `LOC_LRJ` = 'J2'";
        ...
        public virtual async Task<BaseResult> ORDER_COUNT(BaseParameter BaseParameter)
        {
            BaseResult result = new BaseResult();
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sql = @"SELECT `ORDER_IDX`, `LOC_LRJ`, IFNULL(`PERFORMN`, 0) AS `PERFORMN` , `DSCN_YN`, IFNULL(`T1_TOOL_IDX`, 0) AS `T1_TOOL_IDX`, `ERROR_CHK`  FROM torder_bom_sw `A` WHERE `ORDER_IDX` = '" + ORDER + "'";
                        DataSet ds = await MySQLHelper.FillDataSetBySQLAsync(GlobalHelper.MariaDBConectionString, sql);
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        var VLA1 = BaseParameter.ListSearchString[2];
                        var VLA11 = BaseParameter.ListSearchString[3];
                        var C11_D01 = BaseParameter.ListSearchString[4];
                        var TOOL_CONT = BaseParameter.ListSearchString[5];
                        var C11_D02 = BaseParameter.ListSearchString[6];
                        ...
                        string sql = @"UPDATE `torder_bom_sw` SET `PERFORMN`= `PERFORMN` + " + C11_COUNT3 + ", `UPDATE_DTM`= NOW(), `UPDATE_USER`='" + C_USER + "' WHERE  `ORDER_IDX`= '" + ORDER + "' AND `LOC_LRJ` = 'J2'";
                        string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);

                        sql = @"UPDATE ttoolmaster2 SET `TOT_WK_CNT`= `TOT_WK_CNT` + " + C11_COUNT3 + ", `WK_CNT` = `WK_CNT` + " + C11_COUNT3 + ", `UPDATE_DTM`= NOW(), `UPDATE_USER`='" + C_USER + "' WHERE `TOOLMASTER_IDX`= (SELECT ttoolmaster2.`TOOLMASTER_IDX`  FROM ttoolmaster2 WHERE ttoolmaster2.`TOOL_IDX` = (SELECT TTOOLMASTER.`TOOL_IDX` FROM TTOOLMASTER WHERE TTOOLMASTER.`APPLICATOR`  = '" + VLA1 + "') AND ttoolmaster2.SEQ = '" + VLA11 + "')";
                        ...
                        sql = @"INSERT INTO TWWKAR_LP (`PART_IDX`, `WK_QTY`, `CREATE_DTM`, `CREATE_USER`, `MC_NO`, `TORDER_IDX`, `WK_TERM`)  VALUES ('" + C11_D01 + "', " + C11_COUNT3 + ", NOW(), '" + C_USER + "', '" + C11_D02 + "', '" + ORDER + "', 'RH_JOINT')";
                        sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                    }
