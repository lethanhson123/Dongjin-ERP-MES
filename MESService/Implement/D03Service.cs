    ...
    , ID03Service
    {
        ...
        public virtual async Task<BaseResult> Buttonfind_Click(BaseParameter BaseParameter)
        {
            ...
            try
            {
                ...
                {
                    string QRCode = BaseParameter.Barcode.ToUpper().Trim();
        ...
        private async Task<BaseResult> LoadPackingList(string QRCode)
        {
            ...
            if (QRCode.Contains("3S") && QRCode.Contains("PKI"))
            {
        ...
        private async Task<BaseResult> LoadHistory(BaseParameter baseParameter)
        {
            ...
            baseParameter.PartNo = baseParameter.PartNo.Trim();
            ...
            if (baseParameter.FilterType == "Y")
            {
                query = $@"SELECT mainTB.* From
                        ...
                        WHERE mainTB.`PKG_GRP` LIKE '{LPARKNO}' 
                              OR   mainTB.`PART_NO` LIKE '{LPARKNO}'  
                              OR   mainTB.`PART_CAR` LIKE '{LPARKNO}' 
                              OR   mainTB.`PART_NM` LIKE '{LPARKNO}'    
                              OR   mainTB.`LOT_CODE` LIKE '{LPARKNO}'    
            ...
            else if (baseParameter.FilterType == "N")
            {
                query = $@"SELECT mainTB.* From
                        ...
                        LEFT JOIN tiivtr ON tiivtr.PART_IDX = tdpdmtim.VLID_PART_IDX AND tiivtr.LOC_IDX = '2'
                        WHERE 
                            tdpdmtim.VLID_DTM BETWEEN '{ADATE}' AND '{BDATE}' and tdpdmtim.VLID_DSCN_YN ='N') as mainTB
                        WHERE mainTB.`PKG_GRP` LIKE '{LPARKNO}' 
                              OR   mainTB.`PART_NO` LIKE '{LPARKNO}'  
                              OR   mainTB.`PART_CAR` LIKE '{LPARKNO}' 
                              OR   mainTB.`PART_NM` LIKE '{LPARKNO}'   
            ...
            else if (baseParameter.FilterType == "D")
            {
                ...
                Where mainTB.PART_NO LIKE '{LPARKNO}'
                    OR mainTB.PART_CAR LIKE '{LPARKNO}'
                    OR mainTB.PART_NM LIKE '{LPARKNO}'
                    OR mainTB.PKG_GRP LIKE '{LPARKNO}'
                    OR mainTB.`LOT_CODE` LIKE '{LPARKNO}' 
                ORDER BY mainTB.MTIN_DTM DESC, mainTB.PART_NO, mainTB.PKG_GRP ";
            }
            else
            ...
            result.ListtspartTranfer = await MySQLHelperV2.QueryToListAsync<tspartTranfer>(GlobalHelper.MariaDBConectionString, query);
            //result.ListtspartTranfer = new List<tspartTranfer>();
        ...
        public virtual async Task<BaseResult> Buttonadd_Click(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(BaseParameter.Barcode))
                {
                    string barcode = BaseParameter.Barcode.ToUpper().Trim();
                    ...
                    {
                        result = await AddNewIDCardAsync(barcode, partNo, User_ID, BaseParameter.CustomerCode);
                    }
                }
            }
            catch (Exception ex)
        ...
        private async Task<BaseResult> AddNewIDCardAsync(string barcode, string partNo, string User_ID, string customerCode)
        {
            BaseResult result = new BaseResult();
            ...
            else //chưa được Scan vào tức là Scan mới
            {
                ...
                AddtoDatabase(result, barcode, IDCard, User_ID);
        ...
        private async Task<BaseResult> AddMoretoIDCardAsync(string barcode, string partNo, string IDCard, string Create_User)
        {
            ...
            // lấy thông tin sản phẩm
            string sql = @"SELECT tspart.`PART_IDX`, tspart.`PART_NO`, tspart.`PART_NM`, tspart.`PART_CAR`, tspart.`PART_FML`, tspart.`PART_SNP`, 
                                    (SELECT TSCODE.`CD_SYS_NOTE` FROM TSCODE WHERE TSCODE.`CD_IDX` = tspart.`PART_SCN`) AS `PART_SCN_Name`, 
                                    IFNULL((SELECT tiivtr.`QTY` FROM tiivtr WHERE tspart.`PART_IDX` = tiivtr.`PART_IDX` AND tiivtr.`LOC_IDX` = '1'),0) AS `QTY`
                                    FROM tspart WHERE tspart.`PART_USENY` = 'Y' AND (tspart.`PART_NO` = '" + partNo + "' OR tspart.`PART_SUPL` = '" + partNo + "')";

            result.ListtspartTranfer = await MySQLHelperV2.QueryToListAsync<tspartTranfer>(GlobalHelper.MariaDBConectionString, sql);
            ...
            // lấy thông tin lịch sử scan hàng
            sql = @"Select * FROM  tdpdmtim  WHERE tdpdmtim.`VLID_BARCODE` = '" + barcode + "'";
            result.ListtdpdmtimTranfer = await MySQLHelperV2.QueryToListAsync<tdpdmtimTranfer>(GlobalHelper.MariaDBConectionString, sql);
