...
{
    public class C11_ER_LService : BaseService<torderlist, ItorderlistRepository>
    , IC11_ER_LService
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C11_ER_LService(ItorderlistRepository torderlistRepository
        ...
        public virtual async Task<BaseResult> C11_ERROR_Load(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    string sql = @"INSERT INTO `TSNON_OPER` (`TSNON_OPER_MCNM`, `TSNON_OPER_USERNM`, `TSNON_OPER_DATE`, `TSNON_OPER_STIME`, `CREATE_DTM`, `CREATE_USER`, `TSNON_OPER_CODE`,`REMARK`) VALUES ('" + MCbox + "', '" + USER + "', '" + DateTime.Now.ToString("yyyy-MM-dd") + "', NOW(), '" + ST_TM + "', '" + USER + "', 'S', 'New MES C11 ER L')";
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sql = @"UPDATE `TSNON_OPER` SET `TSNON_OPER_ETIME`= NOW(), `UPDATE_DTM`= NOW(), `UPDATE_USER`='" + USER + "', `TSNON_OPER_TIME` = TIME_TO_SEC(TIMEDIFF(`TSNON_OPER_ETIME`, `TSNON_OPER_STIME`)) WHERE  `TSNON_OPER_IDX`= '" + LAST_ID + "'  and `TSNON_OPER_ETIME` is null;";
                        string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
        ...
        public virtual async Task<BaseResult> Button1_ClickSub(BaseParameter BaseParameter)
        {
            BaseResult result = new BaseResult();
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sqlResult = "";
                        string sql = @"SELECT `BARCD_ID`, `DSCN_YN`  FROM  TMBRCD   WHERE   `BARCD_ID` = '" + AA + "'";
                        DataSet ds = await MySQLHelper.FillDataSetBySQLAsync(GlobalHelper.MariaDBConectionString, sql);
                ...
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        if (IsCheck == true)
                        {
                            ...
                            sql = @"UPDATE torder_bom_sw  SET `T1_TOOL_IDX` = '" + TOOLMASTER_IDX + "' WHERE  `ORDER_IDX` ='" + Label1 + "' AND `LOC_LRJ` = 'L'";
                            sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        }
                    }
                }
        ...
        public virtual async Task<BaseResult> Button2_Click(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    string sql = @"UPDATE `torder_bom_sw` SET `ERROR_CHK`='Y', `UPDATE_DTM` = NOW(), `UPDATE_USER` = '" + USER_ID + "' WHERE  `ORDER_IDX`='" + Label1 + "'  AND `LOC_LRJ` = 'L'";
                    string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                }
            }
            catch (Exception ex)
