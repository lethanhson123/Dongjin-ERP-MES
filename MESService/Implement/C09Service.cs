namespace MESService.Implement
{
    ...
    , IC09Service
    {
        ...
        public virtual async Task<BaseResult> Buttonfind_Click(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    {
                        var AA = DateTime.Parse(BaseParameter.ListSearchString[0]).AddDays(-11).ToString("yyyy-MM-dd");
                        ...
                        var FCT = BaseParameter.ListSearchString[4];
                        string sql = @"SELECT  'FALSE' AS `CHK`, 
                        IF(LEFT(`OR_NO`, 1)='N','','E') AS `OR_NO`, `WORK_WEEK`, `ORDER_IDX`, 
                        
                        IF(IFNULL(SUM(CASE WHEN `CONDITION` = 'Stay' THEN 1 END), 1) + 
            	            IFNULL(SUM(CASE WHEN `CONDITION` = 'Working' THEN 1 END), 0) = 0, 'Close',
            	            IF(IFNULL(SUM(`PERFORMN`), 0) = 0, 'Stay', IF(IFNULL(SUM(`PERFORMN`), 0) >= IFNULL(SUM(`PO_QTY`), 0), 'Complete', 'Working'))) AS `CONDITION`, 

                        `CREATE_DTM`, `LEAD_NO`, SUM(`TOEXCEL_QTY`) AS `TOEXCEL_QTY`, 
                        IFNULL((SELECT `QTY` FROM  tiivtr_lead  WHERE `PART_IDX` = (SELECT `LEAD_INDEX` FROM torder_lead_bom WHERE `LEAD_PN` = TORDERLIST_SPST.`LEAD_NO`)), 0) AS QTY_STOCK,
                        `SAFTY_QTY`,  SUM(`PO_QTY`) AS `PO_QTY`, SUM(`PERFORMN`) AS `PERFORMN`, 
                        `MC`,  `PO_DT`,  `BUNDLE_SIZE`, 
                        (SELECT `HOOK_RACK` FROM  trackmaster  WHERE trackmaster.`LEAD_NO` = TORDERLIST_SPST.`LEAD_NO`) AS `HOOK_RACK`, 
                        `LEAD_COUNT`

                        FROM  TORDERLIST_SPST 

                        WHERE `DSCN_YN` = 'Y' AND `PO_YN`='Y' AND   TORDERLIST_SPST.`LEAD_NO` LIKE '" + CC + "' AND  TORDERLIST_SPST.`CONDITION` LIKE '" + DD + "'    AND(TORDERLIST_SPST.PO_DT >= '" + AA + "')                  AND TORDERLIST_SPST.`MC` = '" + BB + "' AND TORDERLIST_SPST.`FCTRY_NM` = '" + FCT + "'            GROUP BY TORDERLIST_SPST.`LEAD_NO`, `OR_NO`            ORDER BY  `CONDITION` DESC,  `LEAD_NO`,  `CREATE_DTM`   ";
                        ...
                        if (result.DataGridView1.Count > 0)
                        {
                            ...
                            {
                                var totalValue = result.DataGridView1[i].TOEXCEL_QTY;
                                var doneValue = result.DataGridView1[i].QTY_STOCK;
                                var remainingValue = totalValue - doneValue;
                                result.DataGridView1[i].QTY_REM = remainingValue;
                            }
                        }
                    }
                }
                        ...
                        if (BaseParameter.DataGridView1.Count > 0)
                        {
                            foreach (var item in BaseParameter.DataGridView1)
                            {
                                if (item.CHK == true)
                                {
                                    ...
                                    var BB = item.MC;
                                    VAL = "UPDATE TORDERLIST_SPST SET `MC`='" + BB + "' WHERE `LEAD_NO` = '" + AA + "' AND  `CREATE_DTM` = '" + AAA + "' ";
                                    string sql = VAL;
                ...
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.DataGridView1 != null)
                    {
                        if (BaseParameter.DataGridView1.Count > 0)
                        {
                            ...
                            List<string> LD_CHK = new List<string>();
                            foreach (var item in BaseParameter.DataGridView1)
                            {
                                if (item.CHK == true)
                                {
                                    var CREATE_DTM = item.CREATE_DTM.Value.ToString("yyyy-MM-dd HH:mm:ss");
                                    var LEAD_NO = item.LEAD_NO;
                                    CHK.Add(CREATE_DTM);
                                    LD_CHK.Add(LEAD_NO);
                                    CONT = CONT + 1;
                                }
                            ...
                            if (CONT > 0)
                            {
                                ...
                                {
                                    string sql = @"UPDATE TORDERLIST_SPST SET `CONDITION`='Close', `UPDATE_DTM` = NOW(), `UPDATE_USER` = '" + USER_IDX + "'   WHERE  NOT(`CONDITION`='Complete') AND   `LEAD_NO` = '" + LD_CHK[GG] + "' AND  `CREATE_DTM` = '" + CHK[GG] + "'  ";
                                    await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                                }
                            }
                        }
        ...
        public virtual async Task<BaseResult> C09_Load(BaseParameter BaseParameter)
        {
            BaseResult result = new BaseResult();
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    //string sql = @"INSERT INTO tsnon_oper_mitor (`tsnon_oper_mitor_MCNM`, `tsnon_oper_mitor_NOIC`, `tsnon_oper_mitor_RUNYN`) VALUES 
                    ...
                    else
                    {
                        sql = @"INSERT INTO tsnon_oper_mitor (`tsnon_oper_mitor_MCNM`, `tsnon_oper_mitor_NOIC`, `tsnon_oper_mitor_RUNYN`,`StopCode`) VALUES 
                                ('" + MCbox + "', '-----', 'N','-')  ON DUPLICATE KEY UPDATE `tsnon_oper_mitor_NOIC` = '-----', `tsnon_oper_mitor_RUNYN` = 'N',`StopCode`='-'";
        ...
        public virtual async Task<BaseResult> TS_USER(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    {
                        var DATEMIN = result.Search1[0].DATEMIN;
                        result.Search1[0].Name = DATEMIN.Value.ToString("HH:mm:ss");
                        result.Search1[0].Description = DATEMIN.Value.ToString("yyyy-MM-dd HH:mm:ss");
                    }
                }
            }
            catch (Exception ex)
        ...
        public virtual async Task<BaseResult> Button1_Click_1(BaseParameter BaseParameter)
        {
            ...
            try
            {
                ...
                string sql = @"SELECT `LEAD_NO`, SUM(`PO_QTY`) AS `TOTAL_QTY`  FROM   TORDERLIST_SPST   
                WHERE `CONDITION` = 'STAY' AND (`MC` LIKE 'Z1%' OR `MC` LIKE 'PLAN')
                GROUP BY `LEAD_NO`";
                DataSet ds = await MySQLHelper.FillDataSetBySQLAsync(GlobalHelper.MariaDBConectionString, sql);
