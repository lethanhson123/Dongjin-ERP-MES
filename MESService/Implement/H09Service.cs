...
{
    public class H09Service : BaseService<torderlist, ItorderlistRepository>
    , IH09Service
    {
        private readonly ItorderlistRepository _torderlistRepository;
        public H09Service(ItorderlistRepository torderlistRepository
        ...
        public virtual async Task<BaseResult> Load()
        {
            ...
            try
            {
                ...
                string sqlCheck = @"SELECT `TSNON_OPER_MCNM`, `TSNON_OPER_CODE` 
                          ...
                          AND (`TSNON_OPER_MCNM` LIKE 'Z1%' OR `TSNON_OPER_MCNM` LIKE 'S1%') 
    ...
    IFNULL(`TS_TIME_ST`, DATE_FORMAT(NOW(), '%Y-%m-%d 06:00:00')) AS `TS_TIME_ST`,  
...
WHERE `A`.`MC_NO` LIKE 'Z1%' OR `A`.`MC_NO` LIKE 'S1%' ) `AA`

LEFT JOIN
(SELECT `TS_MC_NM`, DATE_FORMAT(MIN(`TS_TIME_ST`), '%Y-%m-%d 06:00:00') AS `TS_TIME_ST`, 
...
AND (`TS_MC_NM` LIKE 'Z1%' OR `TS_MC_NM` LIKE 'S1%' )
...
GROUP BY `MC_NO` WITH ROLLUP";
        ...
        AND (`tsnon_oper_mitor_MCNM` LIKE 'Z1%' OR `tsnon_oper_mitor_MCNM` LIKE 'S1%' OR `tsnon_oper_mitor_MCNM` LIKE 'B%')";
