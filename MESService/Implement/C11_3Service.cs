...
{
    public class C11_3Service : BaseService<torderlist, ItorderlistRepository>
    , IC11_3Service
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C11_3Service(ItorderlistRepository torderlistRepository
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        FROM torder_bom_sw `A` WHERE `ORDER_IDX` = '" + ORDER + "' AND `LOC_LRJ` = 'J'";
                        ...
                        else
                        {
                            ...
                            {
                                sql = @"UPDATE `torder_bom_sw` SET `DSCN_YN`='Y', `UPDATE_DTM` = NOW(), `UPDATE_USER` = '" + USER_ID + "' WHERE  `ORDER_IDX`= '" + ORDER + "'  AND `LOC_LRJ` = 'J'";
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sql = @"UPDATE `torder_bom_sw` SET `PERFORMN`= `PERFORMN` + " + C11_COUNT3 + ", `UPDATE_DTM`= NOW(), `UPDATE_USER`='" + C_USER + "' WHERE  `ORDER_IDX`= '" + ORDER + "' AND `LOC_LRJ` = 'J'";
                        string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        ...
                        sql = @"INSERT INTO TWWKAR_LP (`PART_IDX`, `WK_QTY`, `CREATE_DTM`, `CREATE_USER`, `MC_NO`, `TORDER_IDX`, `WK_TERM`) VALUES ('" + C11_D01 + "', " + C11_COUNT3 + ", NOW(), '" + C_USER + "', '" + C11_D02 + "', '" + ORDER + "', 'LH_JOINT')";
                        sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                    }
