...
{
    public class C11_2Service : BaseService<torderlist, ItorderlistRepository>
    , IC11_2Service
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C11_2Service(ItorderlistRepository torderlistRepository
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        FROM torder_bom_sw `A` WHERE `A`.`ORDER_IDX` = '" + ORDER + "' AND `A`.`LOC_LRJ` = 'R'";
                        ...
                        else
                        {
                            ...
                            {
                                sql = @"UPDATE `torder_bom_sw` SET `DSCN_YN`='Y', `UPDATE_DTM` = NOW(), `UPDATE_USER` = '" + USER_ID + "' WHERE  `ORDER_IDX`= '" + ORDER + "'  AND `LOC_LRJ` = 'R'";
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sql = @"SELECT `COLSIP` FROM torderinspection_sw WHERE `ORDER_IDX` = '" + ORDER + "' AND `LOC_LRJ` = 'R'";
                        DataSet ds = await MySQLHelper.FillDataSetBySQLAsync(GlobalHelper.MariaDBConectionString, sql);
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        string sql = @"UPDATE `torder_bom_sw` SET `PERFORMN`= `PERFORMN` + " + C11_COUNT3 + ", `UPDATE_DTM`= NOW(), `UPDATE_USER`='" + C_USER + "' WHERE  `ORDER_IDX`= '" + ORDER + "' AND `LOC_LRJ` = 'R'";
                        string sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
