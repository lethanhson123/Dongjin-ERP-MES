...
{
    public class C05_ER_LService : BaseService<torderlist, ItorderlistRepository>
    , IC05_ER_LService
    {
        private readonly ItorderlistRepository _torderlistRepository;
        private readonly IWebHostEnvironment _WebHostEnvironment;
        public C05_ER_LService(ItorderlistRepository torderlistRepository
        ...
        public virtual async Task<BaseResult> C11_ERROR_Load(BaseParameter BaseParameter)
        {
            ...
            try
            {
                if (BaseParameter != null)
                {
                    ...
                    string sql = @"INSERT INTO `TSNON_OPER` (`TSNON_OPER_MCNM`, `TSNON_OPER_USERNM`, `TSNON_OPER_DATE`, `TSNON_OPER_STIME`, `CREATE_DTM`, `CREATE_USER`, `TSNON_OPER_CODE`, `REMARK`) VALUES ('" + MCbox + "', '" + USER_ID + "', DATE_FORMAT(NOW(), '%Y-%m-%d'), NOW(), '" + ST_TM + "', '" + USER_ID + "', 'S','New MES C05 ER L')";
                ...
                if (BaseParameter != null)
                {
                    ...
                    if (BaseParameter.ListSearchString != null)
                    {
                        ...
                        if (IsCheck == true)
                        {
                            ...
                            sql = @"UPDATE torder_bom_LP  SET `T1_TOOL_IDX` = '" + TOOLMASTER_IDX + "' WHERE  `ORDER_IDX` ='" + Label1 + "'";
                            sqlResult = await MySQLHelper.ExecuteNonQueryAsync(GlobalHelper.MariaDBConectionString, sql);
                        }
                    }
                }
