...
{
    public class V03_3Service : BaseService<torderlist, ItorderlistRepository>
    , IV03_3Service
    {
        private readonly ItorderlistRepository _torderlistRepository;
        public V03_3Service(ItorderlistRepository torderlistRepository
        ...
        public virtual async Task<BaseResult> ComboBox1_Changed(BaseParameter BaseParameter)
        {
            BaseResult result = new BaseResult();
            try
            {
                ...
                {
                    string selectedValue = BaseParameter.ListSearchString[0];
                    ...
                    string sql = @"SELECT 
                        ...
                        `B`.`PQTY`, `A`.`PDP_QTY`, `A`.`PDP_MEMO`, `A`.`CREATE_DTM`, `A`.`CREATE_USER`, 
                    ...
                    WHERE `A`.`PDP_REC_YN` = 'Y' 
                    AND `A`.`PDP_CONF` = 'Report' 
                    AND `PDP_CNF_YN` = 'N' 
                    AND `A`.PDP_NO = '" + selectedValue + "'";
